...
internal class DeepLinkMatcher<T : Any>(
     ...
     *
     * Returns a map of decoded arguments if this matches the pattern, otherwise null.
     */
    fun match(): Map<String, Any>? {
        if (request.uri.getScheme() != deepLinkPattern.uriPattern.getScheme()) return null
        if (!request.uri.getAuthority().equals(
                deepLinkPattern.uriPattern.getAuthority(),
                ignoreCase = true
            )
        ) return null
