...
/**
 * Parse a supported deeplink and stores its metadata as an easily readable format
 *
 ...
 *
 * @param T the backstack key type that supports the deep linking of [uriPattern]
...
internal class DeepLinkPattern<T : Any>(
    ...
    val pathSegments: List<PathSegment> = buildList {
        uriPattern.getPathSegments().forEach { segment ->
            ...
            if (result != null) {
                ...
                if (elementIndex == CompositeDecoder.UNKNOWN_NAME) {
                    throw IllegalArgumentException(
                        "Path parameter '{$argName}' defined in the Nav3cDeepLink $uriPattern does not exist in the Serializable class '${serializer.descriptor.serialName}'."
                    )
                }
